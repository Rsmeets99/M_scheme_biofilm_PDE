...
'''
Python script for computing the convergence rate to the exact solution with respect to the time-step size.
'''
...
def generate_data(dt_list: list[float], 
    ...
    for i, dt in enumerate(dt_list):
        ...
        utils.pme_M_scheme(solution, general_config, parsed_args)
...
def combine_data(dt_list: list[float], 
    ...
    path_name = op.join(example_dir, f'conv_time_results.txt')
...
def plot_data(geometry_config: utils.geometry_param, 
    ...
    path_name = op.join(example_dir, f'conv_time_results.txt')
    path_name_fig = op.join(example_dir, f'conv_time_results_figure.png')
    ...
    plt.plot(dt_data, conv_data, 'r.', label = 'Error')
    ...
    plt.ylabel('Error')
    plt.title(f'Error against time step, $h = {geometry_config.h:.2E}$, $\\gamma = {solution_config.gamma:.2f}$, {geometry_config.dim:.0f}D, $m = {solution_config.m:.0f}$')
    plt.grid()
    plt.legend()
...
if __name__ == '__main__':
    ...
    dt_list = config.experiment_param_1D_conv_alpha_pme.dt_list

    geometry_config = config.geometry_param_1D_conv_time_pme
    solution_config = config.solution_param_1D_conv_time_pme
    general_config = config.general_param_1D_conv_time_pme
