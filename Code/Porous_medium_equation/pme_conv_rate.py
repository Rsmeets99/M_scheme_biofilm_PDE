...
def generate_data(dt_list: list[float], 
    ...
    for i, dt in enumerate(dt_list):
        ...
        solution_config.final_time = solution_config.start_time + dt
...
def plot_data(geometry_config: utils.geometry_param, 
    ...
    plt.title(f'Convergence rate, $h = {geometry_config.h:.2E}$, $\\gamma = {solution_config.gamma:.2f}$, {geometry_config.dim:.0f}D, $m = {solution_config.m:.0f}$')
    plt.grid()
    plt.legend()
...
if __name__ == '__main__':
    ...
    geometry_config = config.geometry_param_1D_conv_alpha_pme
    solution_config = config.solution_param_1D_conv_alpha_pme
    general_config = config.general_param_1D_conv_alpha_pme
